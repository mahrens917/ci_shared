...
Mirror AGENTS.md here—same rules, automation-focused. ci_shared is the canonical CI/guard toolkit for Zeus/Kalshi/API/AWS with code in `ci_tools/`, helpers in `scripts/`, configs in `ci_shared.mk` + `shared-tool-config.toml`, tests in `tests/`, docs in `docs/`.
